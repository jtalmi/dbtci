...
@cli.command()
@click.option(
    "--macros", help="Run and/or test models using changed macros", is_flag=True
)
...
def run_changed(check_macros, children, full_refresh, test, debug):
    ...
    dbt_manager.execute_changed(
        "run", check_macros, children, full_refresh, test, debug
    )
...
@click.option(
    "--yml", help="List of required yml fields for each resource", is_flag=True
)
def lint(resource_type="model", tests=DEFAULT_REQUIRED_TESTS, yml=DEFAULT_REQUIRED_YML):
    ...
    if resource_type == "model":
        models_without_tests = dbt_manager.lint(
            "model", dbt_manager.check_schema_tests, required=tests
        )
        models_without_yml = dbt_manager.lint(
            "model", dbt_manager.check_yaml, required=yml
        )
    elif resource_type == "macro" and "description" in yml:
        macros_without_yml = dbt_manager.lint(
            "macro", dbt_manager.check_yaml, required=["description"]
        )

    if models_without_tests or models_without_yml or macros_without_yml:
        if models_without_tests:
            msg = (
                "The following models do not have the required tests: \n   - %s"
                % "\n   - ".join(models_without_tests)
            )
            click.secho(msg, fg="red")
        if models_without_yml:
            msg = (
                "The following models do not have valid YAML: \n   - %s"
                % "\n   - ".join(models_without_yml)
            )
            click.secho(msg, fg="red")
        if macros_without_yml:
            msg = (
                "The following macros do not have valid YAML: \n   - %s"
