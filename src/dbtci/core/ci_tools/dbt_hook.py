...
class DbtHook:
    ...
    def model_manifest(self, list_of_models: List=[], exclude_tags: List=[]):
      ...
      if exclude_tags:
        model_dicts = {k: v for k, v in model_dicts.items() if not intersection_is_not_empty(v['tags'], exclude_tags)}
    ...
    def macro_manifest(self, list_of_macros: List=[], exclude_tags: List=[]):
      ...
      if exclude_tags:
        macro_dicts = {k: v for k, v in macro_dicts.items() if not intersection_is_not_empty(v['tags'], exclude_tags)}
    ...
    def _build_options(self, models: List=None, excludes: List=None, dbt_vars: Dict=None, full_refresh: bool=False):
      options = ''
      if models:
